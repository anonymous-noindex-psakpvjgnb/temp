...
def format_results_for_display_simplified(results_dict: Optional[Dict[str, Any]]) -> Tuple[pd.DataFrame, str]:
    """
    REVISED V5.3: Formats SPARQL results into a DataFrame, prioritizing labels over URIs.
    Focuses on readable table output for direct display or LLM summary input.

    Returns:
        Tuple[pd.DataFrame, str]: DataFrame of results, status message string.
    """
    df_empty = pd.DataFrame() # Default empty DataFrame

    if not results_dict:
        logger.warning("Formatting: No results dictionary received.")
        return df_empty, "No results received from execution service."
    if results_dict.get("error"):
        error_msg = f"SPARQL Execution Error: {results_dict['error']}"
        logger.error(f"Formatting: Encountered error from execution: {error_msg}")
        return df_empty, error_msg
    ...
    try:
        ...
        if not variables:
             logger.warning("Formatting: SPARQL query returned no variables in the SELECT clause.")
             return df_empty, "Query returned no variables." # More informative message
        if not bindings:
            logger.info("Formatting: Query executed successfully, but returned no results (bindings).")
            return df_empty, "Query executed successfully, but returned no results."

        processed_data = []
        # Create sets for quick lookup: vars selected and potential label vars
        vars_set = set(variables)
        # Map base variable name to its corresponding label variable name IF the label var exists in the SELECT clause
        label_vars_map = {var: f"{var}Label" for var in variables if f"{var}Label" in vars_set}
        logger.debug(f"Formatting: Variables={variables}, LabelVarMap={label_vars_map}")

        for binding in bindings:
            row = {}
            for var in variables:
                # Skip label variables themselves if they are mapped; process them with their base variable
                if var in label_vars_map.values():
                    continue

                value_info = binding.get(var)
                display_value = None

                # Check if a corresponding Label variable exists FOR THIS VAR and has a value in THIS BINDING
                label_var_name = label_vars_map.get(var) # e.g., get 'itemLabel' if var is 'item'
                label_info = binding.get(label_var_name) if label_var_name else None

                if label_info and label_info.get("value"):
                    # Priority 1: Use the label's value
                    display_value = label_info.get("value")
                    # logger.debug(f"Var '{var}': Using Label '{label_var_name}' value '{display_value}'")
                elif value_info:
                    # Priority 2: Fallback to the raw value
                    display_value = value_info.get("value")
                    # Basic cleanup for URIs if it was a URI type and we didn't get a label
                    value_type = value_info.get("type")
                    if value_type == "uri":
                        uri = display_value
                        if "wikidata.org/entity/" in uri: display_value = f"wd:{uri.rsplit('/', 1)[-1]}"
                        elif "dbpedia.org/resource/" in uri: display_value = f"dbr:{uri.rsplit('/', 1)[-1]}"
                        elif "dbpedia.org/ontology/" in uri: display_value = f"dbo:{uri.rsplit('/', 1)[-1]}"
                        elif "dbpedia.org/property/" in uri: display_value = f"dbp:{uri.rsplit('/', 1)[-1]}"
                    # logger.debug(f"Var '{var}': Using Raw value '{display_value}' (Label missing or empty)")
                # If neither value nor label info exists (e.g., unbound variable), leave as None/NaN
                else:
                     logger.debug(f"Var '{var}': No value or label info found in binding.")

                row[var] = display_value # Assign to the original variable name for the DataFrame column

            processed_data.append(row)

        # Create DataFrame from the processed data
        if not processed_data: # Should not happen if bindings exist, but safe check
             return df_empty, "Processing bindings yielded no data."

        df = pd.DataFrame(processed_data)

        # Select only the original variables (not the ?varLabel ones) for the final DataFrame display columns
        display_columns = [v for v in variables if not (v.endswith("Label") and v in label_vars_map.values())]
        # Ensure columns actually exist in the processed DataFrame before selecting
        final_columns = [col for col in display_columns if col in df.columns]

        if final_columns:
            df_final = df[final_columns]
        else: # Fallback if column selection somehow fails
             logger.warning("Could not select display columns, returning DataFrame with all processed columns.")
             df_final = df

        status_msg = f"Query executed successfully. Found {len(bindings)} result(s)."
        logger.info(f"Formatting: Successfully created DataFrame with shape {df_final.shape}")
        return df_final, status_msg

    except Exception as e:
        logger.error(f"Error formatting results: {e}", exc_info=True)
        # Try to return the raw bindings if formatting failed badly
        raw_bindings_str = json.dumps(results_dict.get("results", {}).get("bindings", []), indent=2)
        return df_empty, f"Error during result formatting: {e}. Raw bindings:\n{raw_bindings_str[:1000]}..." # Limit length


# --- Summarization Func (REVISED to use DataFrame/Status) ---
def summarize_results_llm(nlq: str, wd_status: str, wd_df: pd.DataFrame, db_status: str, db_df: pd.DataFrame) -> str:
    """REVISED: Generates a natural language summary based on KG results status and DataFrames."""
    if not llm: logger.error("LLM not available for summarization."); return "Error: LLM not available."

    # Determine if we have meaningful results
    wd_has_data = "successfully" in wd_status.lower() and not wd_df.empty
    db_has_data = "successfully" in db_status.lower() and not db_df.empty

    if not wd_has_data and not db_has_data:
        logger.info("No successful results from either KG to summarize.")
        # Combine status messages for a clearer report
        return f"Could not generate a final answer.\nWikidata Status: {wd_status}\nDBpedia Status: {db_status}"

    logger.info("Step 6: LLM Summarizing results from DataFrames...")

    # Prepare context for summarization prompt using DataFrames (e.g., markdown)
    context = ""
    max_rows_for_prompt = 5 # Limit rows sent to LLM to avoid large prompts

    context += f"Wikidata Status: {wd_status}\n"
    if wd_has_data:
        context += "Wikidata Results Table (Top Rows):\n"
        context += wd_df.head(max_rows_for_prompt).to_markdown(index=False)
        if len(wd_df) > max_rows_for_prompt: context += f"\n(... and {len(wd_df) - max_rows_for_prompt} more rows)"
        context += "\n\n"

    context += f"DBpedia Status: {db_status}\n"
    if db_has_data:
        context += "DBpedia Results Table (Top Rows):\n"
        context += db_df.head(max_rows_for_prompt).to_markdown(index=False)
        if len(db_df) > max_rows_for_prompt: context += f"\n(... and {len(db_df) - max_rows_for_prompt} more rows)"
        context += "\n\n"


    # Summarization Prompt using table data
    prompt_template = PromptTemplate.from_template(
        "Based on the original question and the following results tables retrieved from knowledge graphs, provide a concise, natural language answer. Synthesize the information from both sources if applicable and relevant. If the results seem contradictory or insufficient to answer the question directly, state that clearly.\n\n"
        "Original Question: \"{nlq}\"\n\n"
        "Knowledge Graph Findings:\n{context}\n"
        "Focus on directly answering the question based *only* on the provided table data.\n\n"
    ...
    try:
        ...
        logger.info("   LLM generated consolidated summary from tables.")
        return summary.strip()
    except Exception as e:
        logger.error(f"Error during LLM result summarization: {e}", exc_info=True)
        return "Error: Failed to generate a consolidated summary from results."


def run_complete_pipeline_v5_simplified_format(natural_language_query: str):
    """Orchestrates the full V5 flow with simplified formatting."""
    start_time = time.time()
    logger.info(f"\n--- Pipeline V5.3 Start: '{natural_language_query}' ---")
    ...
    # 3. Generate SPARQL Queries (Using rdfs:label version)
    wd_sparql, db_sparql = generate_sparql_queries_llm_v5(natural_language_query, resolved_entities or {}, resolved_property)

    # 4. Execute Queries & Format Results (Using SIMPLIFIED formatter)
    wd_ok, db_ok = False, False
    wd_results_df, wd_status_msg = pd.DataFrame(), "Execution not attempted."
    db_results_df, db_status_msg = pd.DataFrame(), "Execution not attempted."

    if wd_sparql and wd_sparql != "# N/A":
        logger.info("Pipeline V5.3: Executing Wikidata query via MCP...")
        wd_results_raw = call_mcp_sparql_execution(WIKIDATA_ENDPOINT, wd_sparql, "Wikidata Query Exec")
        wd_results_df, wd_status_msg = format_results_for_display_simplified(wd_results_raw) # <-- Use simplified formatter
        if "successfully" in wd_status_msg.lower() and not wd_results_df.empty: wd_ok = True # Check success *and* non-empty df
    elif not wd_sparql: wd_sparql = "# Query Generation Failed"; wd_status_msg = "Query not generated."

    if db_sparql and db_sparql != "# N/A":
        logger.info("Pipeline V5.3: Executing DBpedia query via MCP...")
        db_results_raw = call_mcp_sparql_execution(DBPEDIA_ENDPOINT, db_sparql, "DBpedia Query Exec")
        db_results_df, db_status_msg = format_results_for_display_simplified(db_results_raw) # <-- Use simplified formatter
        if "successfully" in db_status_msg.lower() and not db_results_df.empty: db_ok = True
    elif not db_sparql: db_sparql = "# Query Generation Failed"; db_status_msg = "Query not generated."

    # 5. Determine Final Answer: Summarize results using DF/Status or use Fallback
    consolidated_answer = ""
    if wd_ok or db_ok:
        logger.info("Pipeline V5.3: Summarizing successful SPARQL results...")
        # Pass DataFrames and status messages to the summarizer
        consolidated_answer = summarize_results_llm(natural_language_query, wd_status_msg, wd_results_df, db_status_msg, db_results_df)
    else:
        logger.warning("Pipeline V5.3: Both queries failed or returned no results. Attempting fallback...")
        consolidated_answer = "SPARQL queries failed or yielded no results. Fallback Answer based on abstracts:\n\n"
        primary_entity_qid, primary_entity_uri = None, None #...(find primary entity - same V5 logic)...
        wd_abstract, db_abstract = None, None #...(fetch abstracts - same V5 logic)...
        if wd_abstract or db_abstract: consolidated_answer += answer_from_abstracts_llm(natural_language_query, wd_abstract, db_abstract)
        else: consolidated_answer += "Fallback failed: Could not identify primary entity or retrieve its abstract."

    end_time = time.time(); logger.info(f"--- Pipeline V5.3 End. Total Time: {end_time - start_time:.2f} seconds ---")

    # Return results for Gradio - Remove triple/graph outputs
    return (
        target_relation_phrase_display, # 0
        resolved_terms_json,            # 1
        wd_sparql,                      # 2
        db_sparql,                      # 3
        wd_results_df if wd_results_df is not None else pd.DataFrame(), # 4 Ensure DF
        db_results_df if db_results_df is not None else pd.DataFrame(), # 5 Ensure DF
        consolidated_answer             # 6
    )


# --- Gradio Interface Definition (REVISED outputs) ---
def create_gradio_interface_v5_simplified():
    """Creates the Gradio UI with simplified results display."""
    logger.info("Creating Gradio interface V5.3 (Simplified Formatting)...")
    with gr.Blocks(theme=gr.themes.Soft(), title="NLQ to KG Agent v5.3") as demo:
        gr.Markdown("# NLQ to Knowledge Graph Agent (v5.3 - Simplified Results)")
        gr.Markdown("Enter query -> Analyze -> Resolve -> Generate/Execute SPARQL -> Summarize/Fallback.")
        ...
        gr.Markdown("## Final Consolidated Answer")
        consolidated_output = gr.Markdown(label="Consolidated Answer") # Final Answer first
        ...
        with gr.Accordion("See Detailed Steps & Results", open=False):
            with gr.Tabs():
                ...
                with gr.TabItem("1. Resolved Terms"):
                     resolved_output = gr.JSON(label="Resolved Entities & Property")
                ...
                # Simplified Results Display - Just the DataFrames
                with gr.TabItem("3. Wikidata Results"):
                     wd_results_df_output = gr.DataFrame(label="Wikidata Results Table", wrap=True, height=400, interactive=False)

                with gr.TabItem("4. DBpedia Results"):
                     db_results_df_output = gr.DataFrame(label="DBpedia Results Table", wrap=True, height=400, interactive=False)

        # Define outputs list matching the return order of the REVISED pipeline function
        outputs = [
            relation_output,        # 0
            resolved_output,        # 1
            wd_sparql_output,       # 2
            db_sparql_output,       # 3
            wd_results_df_output,   # 4
            db_results_df_output,   # 5
            consolidated_output     # 6 - Moved to end to match return order
        ]

        submit_button.click(
            fn=run_complete_pipeline_v5_simplified_format, # Use the pipeline with simplified formatting
            inputs=nlq_input,
            outputs=outputs,
            api_name="query_kg_agent_v5_simple"
        )
        gr.Examples(
            examples=["Who is the president of India?", "What is the population of Paris, France?", "Which actors starred in the movie Inception?", "Show me the chemical formula for caffeine.", "What are the main ingredients in Aspirin?", "Tell me about Alan Turing."],
            inputs=nlq_input,
            fn=run_complete_pipeline_v5_simplified_format,
            # outputs=outputs, # Gradio maps outputs based on return order
